...
func Region_listbyserviceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/regions%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, queryString)
...
func CreateRegion_listbyserviceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_regions",
		mcp.WithDescription("Lists all azure regions in which the service exists."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Region_listbyserviceHandler(cfg),
	}
}
